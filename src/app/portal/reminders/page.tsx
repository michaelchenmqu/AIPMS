...
import { runCheckinReminders, runGasBottleReminders, setGuestPhone } from "./actions";

export default async function RemindersPage({
  searchParams,
}: {
  searchParams: Promise<{ checkin?: string; gas?: string }>;
}) {
  const { checkin, gas } = await searchParams;
  ...
  const [arrivals, jobs] = await Promise.all([
  ...
  return (
    <div>
      <PageHeader title="Reminders" subtitle="Proactive WhatsApp nudges — guest check-in details, and gas-bottle checks for cleaners" />
      ...
      <Card className="p-5">
        ...
        <p className="text-xs text-[var(--color-muted)]">
          Both reminders above run once, on demand. To send them automatically every day, set <code>CRON_SECRET</code>{" "}
          and point a scheduler (a Railway Cron Job, or any external cron) at:
        </p>
