...
import { reviewConditionRoomCheck, returnConditionRoomCheckWithComment } from "@/app/portal/properties/actions";

export function ConditionReviewActions({ roomCheckId }: { roomCheckId: string }) {
  ...
  function approve() {
    startTransition(() => {
      reviewConditionRoomCheck(roomCheckId, "APPROVED");
    });
  }

  function reject() {
    startTransition(() => {
      reviewConditionRoomCheck(roomCheckId, "REJECTED");
    });
  }

  if (showComment) {
    return (
      <form action={returnConditionRoomCheckWithComment} className="flex flex-col gap-2 mt-2">
