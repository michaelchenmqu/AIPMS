...
async function main() {
  ...
  // ---------------------------------------------------------------------
  // Long-term leasing — see prisma/longterm-leasing-seed.ts (shared with
  // the standalone prisma/run-longterm-leasing-seed.ts, which adds the
  // same data to an already-seeded database, e.g. Neon staging/demo).
  // ---------------------------------------------------------------------
  const { morganUser } = await seedLongTermLeasing(prisma, {
    ...
    hinterlandBarn,
    fernGullyCottage,
    leaseTenantPasswordHash: hash("demo1234"),
  });
